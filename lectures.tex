...
$e\,\mathrsfs{L}^T\,hr$ и $e\,\mathrsfs{R}^T\,sh$. Тогда по лемме
...
\section{Отношение равноподсловности}
...
для  каждого $i=0,1,\ldots,p$ (отметим, что $u_0=1$).
...
$u_{i_t}a_i\sim_L u_i\sim_L u_{i_t}.$ Таким образом, мы можем сделать
следующее наблюдение.

\noindent\textbf{Факт 1:} Пусть $u'$ -- подслово слова $u$,
содержащее все вхождения букв на синих позициях (и, возможно, на других).
Тогда $u'\sim_L a_{i_1}a_{i_2}\cdots a_{i_r}\sim_L u.$

Заметим кроме того, что синими индексами помечено самое левое вхождение слова
$u_{\textsf{left}}=a_{i_1}a_{i_2}\cdots a_{i_r}$ как подслова в слово $u$, т.е.
для любого $r'\le r$ слово $a_{i_1}a_{i_2}\cdots a_{i_{r'}}$ не является
подсловом слова $u_{i_{r'}-1}$. Поскольку $u\sim_k v$, то мы можем рассмотреть
самое левое вхождение слова $u_{\textsf{left}}$ в слово $v$. Соответствующие
индексы из множества $\{1,2,\ldots,q\}$ обозначим через
$\bar{i}_1<\bar{i}_2<\cdots <\bar{i}_r$ и назовем синими индексами в слове $v$.

Применим теперь двойственную конструкцию к слову $v$. Рассмотрим \emph{красные}
индексы $j$ в слове $v$, т.е. такие индексы, что $b_jv_{j+1}\nsim v_{j+1}$,
где $v_{j+1}$ -- суффикс слова $v$, начинающийся после $j$-й буквы.
Для красных индексов $j_1<j_2<\cdots<j_s$, $s\le m-1$, справедливо
двойственное свойство, т.е. эти индексы определяют самое правое вхождение
слова $u_{\textsf{right}}=b_{j_1}b_{j_2}\cdots b_{j_s}$ в слово $v$.
Мы будем рассматривать также самое правое вхождение слова $u_{\textsf{right}}$
в слово $u$ и обозначать соответствующие красные индексы через
$\bar{j}_1<\bar{j}_2<\cdots<\bar{j}_s$.

Теперь мы сделаем главный шаг и докажем, что буквы слов $u_{\textsf{left}}$
и $u_{\textsf{right}}$ чередуются в слове $u$ в том же порядке, что и в слове $v$.

\noindent\textbf{Факт 2:} Пусть $\bar{u}$ -- подслово слова $u$, полученное из
букв, стоящих на синих и красных позициях в $u$, а слово $\bar{v}$ -- полученное
аналогичным образом подслово слова $v$. Тогда $\bar{u}=\bar{v}$.

\noindent\emph{Доказательство.} Рассмотрим вхождение буквы $a_{i_{k}}$
на синей позиции $i_k$ и вхождение буквы $b_{j_\ell}$ на красной позиции
$\bar{j}_\ell$ в слове $u$. Предположим сначала, что это разные буквы, т.е.
$a_{i_k}\ne b_{j_{\ell}}$. В этом случае $i_k<\bar{j}_\ell$ тогда и только
тогда, когда слово $$w_{k\ell}=a_{i_1}\cdots a_{i_k}b_{j_\ell}\cdots b_{j_s}=a_{i_1}\cdots a_{i_k}a_{\bar{j}_\ell}\cdots a_{\bar{j}_s}$$
является подсловом слова $u$. Прямая импликация очевидна. Для доказательства
обратной импликации допустим, что $w_{k\ell}\unlhd u$. Пусть $h_1<h_2<\cdots<h_k
<h'_\ell<\cdots<h'_s$ -- соответствующие индексы, т.е. $w_{k\ell}=a_{h_1}\cdots
a_{h_k}a_{h'_\ell}\cdots a_{h'_s}.$ Поскольку синие индексы $i_1,\ldots,i_r$
задают самое левое вхождение слова $u_{\textsf{left}}$ в слово $u$, то индукцией
по $t=1,\ldots,k$ можно доказать, что $i_t\le h_t$. Симметрично, для $t=\ell,\ldots,s$
получаем $h'_t\le \bar{j}_t$. Следовательно, $i_k\le h_k<h'_\ell\le\bar{j}_\ell$.

Теперь рассмотрим случай, когда $a_{i_k}=b_{j_\ell}$.
\begin{enumerate}
\item Если $w_{k\ell}\unlhd u$, то $i_{k}<\bar{j}_\ell$, и обратно;
\item если $w_{k\ell}\ntrianglelefteq u$, но
$w'_{k\ell}=a_{i_1}\cdots a_{i_k}b_{j_\ell+1}\cdots b_{j_s}\unlhd u$, то
$i_k=\bar{j}_\ell$, т.е. рассматриваемый индекс является синим и красным
одновременно;
\item если слово $w'_{k\ell}\ntrianglelefteq u$, то $\bar{j}_\ell<i_k.$
\end{enumerate}
Таким образом, в любом случае взаимное расположение рассматриваемых
синего и красного индексов определяется подсловами слова $u$ длины
не более $k$, поскольку $|w'_{k\ell}|,|w_{k\ell}|\le |u_{\textsf{left}}u_{\textsf{right}}|\le 2m-2=k.$
Итак, утверждение факта 2 следует из условия $u\sim_k v$.

В итоге мы доказали, что $\bar{u}=\bar{v}$, и $\bar{u}\sim_L u$.
Аналогично $\bar{v}\sim_L v$, следовательно, $u\sim_L \bar{u}=\bar{v}\sim_L v$.
\end{proof}

\section{Некоторые следствия теоремы Саймона}

Из теоремы Саймона можно получить неожиданные следствия для теории полугрупп. 
Для начала рассмотрим для каждого натурального $n$ три моноида $\mathcal{C}_n$, $\mathcal{R}_n$ и $\mathcal{U}_n$.
Эти моноиды будут служить в качестве примеров $\mathrsfs{J}$-тривиальных моноидов.

Моноид $\mathcal{C}_n$ состоит из всех сохраняющих порядок направленных преобразований конечной цепи
из $n$ элементов, т.е.\ преобразований $\alpha$ таких, что для любых элементов цепи $p$ и $q$  
$p\le q$ влечет $p\alpha\le q\alpha$ и $p\le p\alpha$. 

Моноид $\mathcal{R}_n$ состоит из всех рефлексивных бинарных отношений на $n$-элементном множестве.
Каждый элемент этого моноида удобно представлять в виде квадратной булевой матрицы порядка $n$ с единицами 
...
\begin{proposition}
\label{J-triv_ex}
При любом натуральном $n$ моноиды $\mathcal{C}_n$, $\mathcal{R}_n$ и $\mathcal{U}_n$ $\mathrsfs{J}$-тривиальны.
\end{proposition}

\begin{proof}
Докажем, что моноид $\mathcal{C}_n$ $\mathrsfs{J}$-тривиален. Пусть $\alpha,\beta\in \mathcal{C}_n$ и $\alpha\mathrsfs{J}\beta$.
Тогда $\alpha=\gamma\beta\delta$ и $\beta=\sigma\alpha\tau$ для некоторых $\gamma,\delta,\sigma,\tau\in \mathcal{C}_n$. Поскольку 
$\gamma$ -- направленное преобразование, имеем $p\le p\gamma$. Далее $p\beta\le p\gamma\beta$, так как $\beta$ сохраняет порядок. 
Пользуясь тем, что $\delta$ -- направленное преобразование, получаем $p\beta\le p\gamma\beta\le p\gamma\beta\delta=p\alpha$. 
Рассуждая аналогичным образом, получаем $p\alpha\le p\beta$, следовательно, $\alpha=\beta$, т.е.\ моноид $\mathcal{C}_n$ $\mathrsfs{J}$-тривиален.

Докажем следующее вспомогательное утверждение: \emph{если на моноиде $M$ можно ввести отношение частичного порядка $\le$, стабильное относительно операции
моноида, и такое, что для всех $x\in M$ выполняется неравенство $x\le 1$, то $M$ $\mathrsfs{J}$-тривиален.} Действительно, пусть $x\mathrsfs{J} y$. 
Тогда существуют $a,b,c,d\in M$ такие, что $x=ayb$, $y=cxd$. Из того, что  $a\le 1$, $b\le 1$ и порядок стабилен относительно операции моноида, следует
$x=ayb\le y$. Аналогично $y\le x$. Следовательно, $x=y$.

Отметим, что поскольку $\mathcal{U}_n$ -- подмоноид моноида $\mathcal{R}_n$, достаточно доказать, что моноид $\mathcal{R}_n$ $\mathrsfs{J}$-тривиален.
Для этого достаточно заметить, что моноид $\mathcal{R}_n$ естественным образом упорядочен отношением включения. Очевидно, этот порядок стабилен относительно произведения
бинарных отношений. Кроме того, единица моноида (диагональ $\delta_n$) содержится в любом рефлексивном отношении.
\end{proof}

Будем говорить, что моноид $M$ \textit{делит} моноид $N$, если $M$ является гомоморфным образом 
некоторого подмоноида моноида $N$.
Из теоремы Саймона следует следующая теорема. 
\begin{theorem}[Страубинг-Пэн]
Пусть M -- конечный моноид. Следующие условия эквивалентны:

(1) $M$ -- $\mathrsfs{J}$-тривиальный моноид,
...
\chapter{Теорема Шютценберже}

Характеризация беззвездных языков, полученная Шютценберже в 1965 году, -- второй по важности (после теоремы Клини)
результат в теории конечных автоматов.
...
алфавитом $\Sigma$ -- это наименьший класс языков, который
\begin{itemize}
\item[а)] содержит пустой язык, язык $\{1\}$ и все языки вида $\{a\}$, где $a\in\Sigma$;
\item[б)] вместе с любым языком $L$ содержит его дополнение $L^C$;
\item[в)] вместе с любыми двумя языками содержит их объединение и
их произведение.
\end{itemize}
...
Таким образом, определение класса беззвездных
языков получается из определения класса регулярных языков заменой операции итерации на дополнение. 
Поскольку класс регулярных языков замкнут относительно взятия дополнения, то любой беззвездный язык
является регулярным, но, как мы увидим позднее, обратное неверно. Из определения также немедленно 
следует, что любой конечный язык беззвездный.

\begin{example}
Пусть $X\subseteq\Sigma$. Тогда язык $\Sigma^*X\Sigma^*$ является беззвездным, поскольку $\Sigma^*=\varnothing^C$.
Язык $X^*$ также является беззвездным, поскольку 
$$X^*=\Sigma^*\setminus \bigcup\limits_{a\in\Sigma\setminus X}\Sigma^*a\Sigma^*=\left(\bigcup\limits_{a\in\Sigma\setminus X}\varnothing^Ca\varnothing^C\right)^C.$$
\end{example}


\section{Теорема Шютценберже}

Конечный моноид $M$ называется \index[index]{моноид!апериодический}\emph{апериодическим}, если существует натуральное $n$ такое, что $x^{n+1}=x^n$ для любого $x\in M$.
Наименьшее $n$ с таким свойством будем называть \emph{экспонентой} моноида $M$.

\begin{lemma} Конечный моноид $M$ $\mathrsfs{H}$-тривиален тогда и только тогда, когда $M$ апериодический.
\end{lemma}

\begin{proof}
Пусть моноид $M$ конечен и $\mathrsfs{H}$-тривиален. Рассмотрим произвольное $x\in M$. 
Поскольку $M$ конечен, $x^{n}=x^{n+k}$ для некоторых $n$  и $k$. Тогда очевидно, что $x^n$ и $x^{n+1}$
находятся в одном $\mathrsfs{H}$-классе. Следовательно $x^n=x^{n+1}$. Число $n$ в этом равенстве, вообще говоря, 
свое для каждого $x\in M$. Но так как моноид конечен, в определении апериодичности можно взять наибольшее из всех таких $n$.

Обратно, пусть моноид $M$ апериодический. Рассмотрим $x,y\in M$ такие, что $x\mathrsfs{H} y$. Это значит, что
$x=ay$, $y=bx$, $x=yc$, $y=xd$ для некоторых $a,b,c,d\in M$. Тогда $x=ay=axd=a^2xd^2=\ldots=a^nxd^n$ для произвольного $n$.
В частности, при $n$, равном экспоненте моноида, получаем $y=xd=a^nxd^{n+1}=a^nxd^n=x$. Следовательно, моноид $M$ 
$\mathrsfs{H}$-тривиален.
\end{proof}
